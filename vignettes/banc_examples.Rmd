...
`ggneuron_gif()` renders a *sequence* of object states (a morph, a developmental
series, or a turntable) to a GIF, reusing `geom_neuron()` for every frame. Each entry
of `flows` is a per-frame list of objects; `volume`/`targets` add static context, and
`alpha` may be given per structure (a translucent brain around solid neurons). The
chunk is `eval = FALSE` (it needs `gifski` or `magick`, and writes a file); the
...
ggneuron_gif(
  ...
  alpha = c(brain = 0.10, neurons = 0.95),
